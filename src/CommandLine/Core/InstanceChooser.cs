...
namespace CommandLine.Core
{
    static class InstanceChooser
    {
        ...
        private static ParserResult<object> MatchVerb(
            ...
            IEnumerable<ErrorType> nonFatalErrors)
        {
            return verbs.Any(a => nameComparer.Equals(a.Item1.Name, arguments.First())|| a.Item1.Alias.Contains(arguments.First()))
                ? InstanceBuilder.Build(
                    Maybe.Just<Func<object>>(
                        () =>
                            verbs.Single(v => nameComparer.Equals(v.Item1.Name, arguments.First()) || v.Item1.Alias.Contains(arguments.First())).Item2.AutoDefault()),
                    tokenizer,
                    arguments.Skip(1),
                    nameComparer,
                    ignoreValueCase,
